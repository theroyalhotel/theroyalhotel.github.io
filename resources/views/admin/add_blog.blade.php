...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
        <h2>{{ isset($edit) ? 'Edit blog' : 'Create blog' }}</h2>
        <ol class="breadcrumb">
            <li>
                <a href=".home">Home</a>
            </li>
            ...
            <li class="active">
                <strong>{{ isset($edit) ? 'Edit blog' : 'Create blog' }}</strong>
            </li>
        </ol>
    </div>
    <div class="col-lg-2">
    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <form action="{{ isset($edit) ? '/admin/blogs/update/'.$edit->id: '/admin/blogs/store' }}" method="POST"
                    ...
                    <div class="form-group"> <label class="col-sm-2 control-label">Detail</label>
                        <div class="col-sm-10">
                            <textarea class="form-control" name="detail" id="detail"
                                required>{{ isset($edit) ? $edit->detail:   ''  }}</textarea>
                        </div>
                    </div>
                    <div class="hr-line-dashed"></div>
                    <div class="form-group"><label class="col-sm-2 control-label">Select A Catagory <br />
                            <a href="/admin/catagories/create"><small class="text-navy">Add new Catagory
                                    here</small></label></a>
                        <div class="col-sm-10">
                            <select name="cata" class="form-control">
                                @foreach ($catagories as $cata)
                                <option value="{{$cata->id}}" @if(isset($edit)) @if($cata->id == $edit->catagory)
                                    selected
                                    @endif
                                    @endif
                                    > {{$cata->name}}
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Select A Tag <br />
                            <a href="/admin/catagories/create"><small class="text-navy">Add new Tag
                                    here</small></label></a>
                        <div class="col-sm-10">

                            @if($tags->count() > 0)
                            <select name="tags[]" id="tags" class="form-control tag" multiple>
                                @foreach ($tags as $tag)
                                <option value="{{$tag->id}}" @if(isset($edit)) @if($edit->hasTag($tag->id))
                                    selected
                                    @endif
                                    @endif
                                    >
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">New Tags</label>
                        <div class="col-sm-10"><input type="text" class="form-control" name="new_tags">
                        </div>
                    </div>
                    @if (isset ($edit))
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Image</label>
                        <div class="col-sm-10"> <input type="file" id="file" class="form-control" name="file">
                        </div>
                    </div>
                    <div class="hr-line-dashed"></div>
                    <div class="form-group">
                        <div class="col-sm-4 col-sm-offset-2">
                            <button class="btn btn-primary"
                                type="submit">{{ isset($edit) ? 'Edit blog' : 'Create blog' }}</button>
                        </div>
                    </div>
                </form>
            </div>
